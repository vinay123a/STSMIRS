...
class EventTriggerEngine:
    ...
    def __init__(self, config_path="config.json"):
        ...
        self.conf_medium = trigger_cfg.get("confidence_medium", 0.70)
    ...
    def process_events(self, action_detections, zone_violations, scoring_engine, persons=None):
        """
        ...
        events = []
        now = time.time()
        num_persons = len(persons) if persons else 0
        
        # 1. Process Action Detections
        for det in action_detections:
            person = det["person"]
            event_type = det["event_type"]
            conf = det["confidence"]
            
            # Enforce 2+ people rule for Fighting
            if event_type == "Fighting" and num_persons < 2:
                continue

            # STRICT WHITELIST: Only Panic, Fall, and Fighting are allowed
            if event_type not in ["Fall", "Fighting", "Panic"]:
                continue
            ...
            if self._can_trigger(person.track_id, event_type, now):
                ...
                # --- NEW: Print to CMD for the user ---
                print("\n" + "="*60)
                print(f" ALERT: {event_type} DETECTED! ")
                print(f" Target: {person.track_label} | Conf: {conf:.2f} ({level})")
                print(f" Location: {person.zone_id or 'Unknown Zone'}")
                print("="*60 + "\n")
        ...
        for viol in zone_violations:
            ...
            if self._can_trigger(person.track_id, event_type, now):
                ...
                print("\n" + "="*60)
                print(f" ALERT: {event_type} DETECTED! ")
                print(f" Target: {person.track_label} | Zone: {zone_id}")
                print("="*60 + "\n")
