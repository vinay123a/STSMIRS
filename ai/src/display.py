...
class DisplayEngine:
    ...
    def draw_persons(self, frame, persons):
        """Draw tracked persons and their identified faces."""
        person_count = len(persons)
        for person in persons:
            # We now draw ALL persons so you can see the tracking and face recognition
            self.draw_person(frame, person, person_count=person_count)
            
            face_bbox = getattr(person, "face_bbox", None) 
            if face_bbox is not None:
                fx1, fy1, fx2, fy2 = face_bbox
                name = getattr(person, "track_label", "Unknown")
                color = (255, 100, 0) if name != "Unknown" and not name.startswith("T-") else (0, 0, 255)
    ...
    def draw_risk_panel(self, frame, persons):
        ...
        for idx, person in enumerate(persons[:rows]):
            ...
            self._draw_metric_bar(frame, x0 + 18, metric_y + metric_gap, panel_w - 36, "FALL", probs.get("Fall", 0.0), (0, 190, 120))
    ...
    def render(self, frame, persons=None, zones=None, fps=0.0,
               external_panic_status=False, external_panic_metrics=None):
        """
        ...
        # Layer 4: External Panic overlay
        if external_panic_metrics:
            h, w = output.shape[:2]
            
            # --- ALWAYS SHOW LIVE METRICS (Small panel) ---
            runners = external_panic_metrics.get("runner_count", 0)
            divergence = external_panic_metrics.get("divergence", 0.0)
            persistence = external_panic_metrics.get("persistence", 0)
            
            panel_w, panel_h = 220, 85
            px, py = w - panel_w - 20, h - panel_h - 20
            cv2.rectangle(output, (px, py), (px + panel_w, py + panel_h), (50, 50, 50), -1)
            cv2.rectangle(output, (px, py), (px + panel_w, py + panel_h), (150, 150, 150), 1)
            
            cv2.putText(output, "PANIC ENGINE LIVE", (px + 10, py + 20),
                        self.font_face, 0.45, (0, 255, 255), 1)
            cv2.putText(output, f"Runners: {runners}", (px + 10, py + 40),
                        self.font_face, 0.4, (255, 255, 255), 1)
            cv2.putText(output, f"Scatter: {divergence:.2f}", (px + 10, py + 55),
                        self.font_face, 0.4, (255, 255, 255), 1)
            cv2.putText(output, f"Persistence: {persistence}", (px + 10, py + 70),
                        self.font_face, 0.4, (255, 255, 255), 1)

            # --- SHOW BIG BANNER ONLY IF PANIC IS ACTIVE ---
            if external_panic_status:
                ptype = external_panic_metrics.get("panic_type", "Panic")
                banner = f"!! CROWD PANIC DETECTED !! [{ptype.upper()} | {runners} runners]"
                banner = self._clean_text(banner)
                bar_h = 46
                y_pos = h - bar_h - 120  # move up so it doesn't overlap the panel
                cv2.rectangle(output, (0, y_pos), (w, y_pos + bar_h), (0, 0, 180), -1)
                text_size, _ = cv2.getTextSize(banner, self.font_face, 0.65, 2)
                tx = (w - text_size[0]) // 2
                cv2.putText(output, banner, (tx, y_pos + 30),
                            self.font_face, 0.65, (255, 255, 255), 2, cv2.LINE_AA)
