...
class SkeletonActionDetector:
    ...
    def predict(self, track_id):
        ...
        # Allow predicting early if we have at least 5 frames (1 second at 5 FPS)
        if len(buffer) < 5:
            return {
                'action': 'Unknown',
                'confidence': 0.0,
        ...
        # Prepare sequence - pad to seq_len by repeating if we have fewer frames
        frames = list(buffer)
        if len(frames) < self.seq_len:
            # Tile the available frames to fill seq_len
            repeats = (self.seq_len + len(frames) - 1) // len(frames)
            frames = (frames * repeats)[:self.seq_len]
        
        # Prepare batch
        sequence = np.array(frames)  # (seq_len, input_dim)
